...
const ViewStaffAcceptedCom = () => {
    ...
    const accept = (id,cId)=>{
        console.log(id);
        axios.get(`http://localhost:2000/api/complaint/complete-complaints-staff/${id}/${cId}`).then((data)=>{
            console.log(data);
            window.location.reload()
            navigate('/staffss-accepted-complaint')
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:2000/api/complaint/view-all-worker-accepted-complaints-staff/${worker_id}`)
          .then((response) => {
...
export default ViewStaffAcceptedCom;
