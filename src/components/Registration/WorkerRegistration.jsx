...
const WorkerRegistration = () => {
  ...
  const validate = (values)=>{
    ...
    if(Object.keys(formErrors).length === 0 && isSubmit){
      axios.post('http://localhost:2000/api/register/worker',inputs).then((data)=>{
        console.log(data);
...
<ToastContainer />
  <form onSubmit={registerSubmit}>
    ...
    />
    <br />
    <span style={{color:"red"}} > {formErrors.area? formErrors.area :""}</span>
    <input
      type="text"
      className="form-control"
      placeholder="Area"
      ...
      value={inputs.area || ""}
      onChange={setRegister} 
      onClick={()=>{setformErrors({...formErrors,area:""})}}
    />
    <br />
